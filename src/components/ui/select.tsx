...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 cursor-pointer items-center justify-between rounded-xl border border-input bg-background px-3 py-2 text-sm ring-offset-background data-[placeholder]:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
const SelectItem = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Item> & { title?: string }
>(({ className, children, title, ...props }, ref) => (
  ...
  >
    ...
    <SelectPrimitive.ItemText>{title ?? children}</SelectPrimitive.ItemText>
    {title ? children : null}
  </SelectPrimitive.Item>
))
