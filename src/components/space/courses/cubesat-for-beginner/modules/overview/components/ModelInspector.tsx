...
import { altitudeToSceneRadius } from "../lib/orbitMath";
...
function EarthPreview() {
  return (
    <mesh>
      ...
      <meshStandardMaterial color="#1e4d7b" emissive="#0a2540" emissiveIntensity={0.35} roughness={0.88} />
    </mesh>
  );
}
...
function SatellitePreview({
  ...
  const model = useMemo(() => {
    // Inspector uses a fixed card-friendly size (not orbital scale)
    const clone = scene.clone(true);
    forceLitMaterial(clone, "#7eb8ff");
    return normalizeToSize(clone, 1.65);
  }, [scene]);
